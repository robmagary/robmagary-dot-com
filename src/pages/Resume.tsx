    ...
    return(
        <ul>
            { workHistory.map((experience) =>
                    <li className='flex flex-col lg:flex-row mb-6' key={experience.employer}>
                        <h3 className='flex flex-col text-xl font-bold lg:w-1/4'>{experience.employer}
                            <small className='text-sm font-normal text-slate-600 leading-tight lg:mt-4'>{experience.dateRange}</small>
                        </h3>
                        <div className='w-3/4'>
                            <div>
                                {(experience.summary && experience.summary.length > 0) ? (
                                    <>
                                        <h5 className='text-normal font-bold lg:w-3/4 mb-4'>Summary</h5>
                                        { experience.summary.map((summaryParagraph) => <p className='mb-4 text-slate-600' key={summaryParagraph.length}>{summaryParagraph}</p> )}
                                        </>
                                ): undefined}
                            </div>
                            <h5 className='text-normal font-bold lg:w-3/4 mb-4'>Responsibilities</h5>
                            <ul className='list-disc pl-8'>
                                {experience.responsibilities.map((responsibility) => <li className='text-slate-600' key={responsibility}>{responsibility}</li>)}
                            </ul>
                        </div>
                    </li>
                )
...
function SectionWrapper({children, title}: PropsWithChildren<SectionProperties>): ReactElement {
    return(
...
export default function ResumePage(): ReactElement {
    return (
        <>
            <Head title='Resume' />
            <Link data-testid='BackLink' to='/' className='flex items-center pb-24'>
            ...
            <div className='flex flex-col px-12'>
                <section className='max-w-screen-lg'>
                    ...
                    <p>
                    A skilled leader with more than four years of experience developing team members and more than ten years of experience designing and developing web and mobile applications. Specializing in casting a product vision, breaking the project down into deployable phases, leading the team to project completion, and building great web applications with Elm.
                    </p>
                </section>
